            ...
            <li key={step}>
              <article className="landing-surface-card flex h-full flex-col p-6">
                <div className="mb-4 flex items-center justify-between">
                  <span className="text-xs font-semibold uppercase tracking-wider text-pv-primary">
                    {role === 'LIVE' ? 'Tiempo real' : `Rol ${role}`}
                  </span>
                  <span className="font-mono text-sm text-pv-muted">{step}</span>
                </div>
                <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg border border-pv bg-pv-surface-soft text-pv-primary">
                  <Icon className="h-5 w-5" aria-hidden />
                </div>
                <h3 className="text-lg font-semibold text-pv-main">{title}</h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-pv-muted">{description}</p>
              </article>
            </li>
          ))}
        </ol>
